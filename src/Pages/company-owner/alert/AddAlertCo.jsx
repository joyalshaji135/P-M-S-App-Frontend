...
function AddAlertCo() {
  ...
  useEffect(() => {
    if (id) {
      ...
      const alertToEdit = storedData.find((alert) => alert.id === parseInt(id));
      if (alertToEdit) {
        setFormData(alertToEdit); // Pre-fill the form with existing data
      }
    }
  }, [id]);
  ...
  const handleSubmit = (e) => {
    ...
    if (id) {
      ...
      const updatedData = storedData.map((alert) =>
        alert.id === parseInt(id) ? formData : alert
      );
      localStorage.setItem('alerts', JSON.stringify(updatedData));
    } else {
