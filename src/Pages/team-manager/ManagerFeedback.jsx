...
function ManagerFeedback() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem('feedbacksMg')) || [];
  ...
  const handleDeleteFeedback = (id) => {
    ...
    localStorage.setItem('feedbacksMg', JSON.stringify(updatedFeedbacks)); // Update localStorage
  };
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex space-x-2">
          {/* View Button */}
          <Link to={`/team-manager/feedbacks/view/${row.id}`}>
          ...
          {/* Edit Button */}
          <Link to={`/team-manager/feedbacks/edit/${row.id}`}>
